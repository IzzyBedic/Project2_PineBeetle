...
```{r recipe}
# recipe
pine_recipe <- pine_train %>% 
  recipe(DeadDist ~ TreeDiam + Infest_Serv1 +  SDI_20th + IND_BA_Infest_20th) %>% 
...
lm(sqrt(DeadDist) ~ TreeDiam + Infest_Serv1 + IND_BA_Infest_20th, data = pine_tbl) %>% 
  glance()
```
...
```{r randomforest}
random_forest <-  rand_forest(trees = 100, mode = "regression") %>%
  set_engine("randomForest") %>%
  fit(DeadDist ~ TreeDiam + Infest_Serv1 + IND_BA_Infest_20th, data = pine_train)
