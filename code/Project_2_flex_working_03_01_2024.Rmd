...
lm(DeadDist_sqrt ~ TreeDiam + Infest_Serv1 + IND_BA_Infest_20th, data = pine_tbl_sqrt) %>% 
  glance()
```
...
ggplot(ridge_predicted, aes(x = DeadDist_sqrt, y = .pred)) + 
...
library(modelr)
pine_tbl_sqrt %>% 
  add_residuals(ridge_fit) %>% 
  ggplot(aes(DeadDist_sqrt, resid) +
  geom_point() +
  geom_abline() +
  geom_errorbar(aes(
    ymin = mean - std_err,
